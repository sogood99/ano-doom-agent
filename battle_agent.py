...
def create_battle_agent(timesteps, show_window=False) -> ppo2.PPO2:
    """
        Create a PPO agent with naive rewards and total training timestep = timestep
    """
    trainEnv = create_vec_env(scenario=DEATHMATCH_CFG, env_param=DEATHMATCH_ENV, reward_type="battle",
                              show_window=show_window, n_envs=4)
    testEnv = create_vec_env(scenario=DEATHMATCH_CFG, env_param=DEATHMATCH_ENV, reward_type="battle",
    ...
    evaluation_callback = callbacks.EvalCallback(testEnv, best_model_save_path='logs/models/battle_agent')

    agent.learn(total_timesteps=timesteps, tb_log_name="battle_agent", callback=evaluation_callback)
    trainEnv.close()
    testEnv.close()
    return agent


def train_existing_battle_agent(timesteps, path, show_window=False) -> ppo2:
    trainEnv = create_vec_env(scenario=DEATHMATCH_CFG, env_param=DEATHMATCH_ENV, reward_type="battle",
                              show_window=show_window, n_envs=4)
    testEnv = create_vec_env(scenario=DEATHMATCH_CFG, env_param=DEATHMATCH_ENV, reward_type="battle",
                             show_window=show_window)
